...
void task_follow_line(void *pvParameters __attribute__((unused))) {
  ...
  float Kp = 57.00;  // Ajusta el valor de Kp según sea necesario
  float Kd = 54.5;  // Ajusta el valor de Kd según sea necesario
  ...
  for (;;) {
    ...
    if (!seen_obstacle){
      ...
      if (sensor_left && !sensor_right) error = -1.0;  // Línea a la izquierda
        ...
        error = (last_err <= 0) ? -3.1 : 3.1; // Gira hacia la izquierda o la derecha
      }
